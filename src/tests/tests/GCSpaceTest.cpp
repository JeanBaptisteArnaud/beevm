...
{
	GCSpace localSpace = flipper->localSpace;
	oop_t *array  = mockedObjects.mockArray(1024);
	oop_t *stTrue = KnownObjects::stTrue;
	oop_t *copy   = localSpace.shallowCopyGrowingTo(array, 2048);
	oop_t *object = localSpace.shallowCopy(stTrue);
	...
	ASSERTM("value not correctly copied", mockedObjects.checkValueMockArray1024(copy));
...
void GCSpaceTest::testGrow() {
	...
	GCSpace localSpace = flipper->localSpace;
	oop_t *array = mockedObjects.mockArray(3); // 100
...
void GCSpaceTest::testGrowingTo() {
	...
	oop_t *copy = localSpace.shallowCopyGrowingTo(array, 2048);
	oop_t *object = localSpace.shallowCopy(KnownObjects::stFalse);
